...
class NotificationManager: ObservableObject {
    ...
    func updateDeviceToken(authToken: String, deviceToken: String) async {
        ...
        let deviceTokenUpdateResult = await app.setNewDeviceToken(token: authToken, deviceToken: deviceToken)
    ...
    func getNotifications(token: String) async {
        ...
        let notificationResult = await app.getNotifications(token: token)
        ...
        case .success(var incomingNotifications):
            ...
            let _ = await app.confirmNotifications(token: token)
        case .failure(_):
            return
        }
    }
