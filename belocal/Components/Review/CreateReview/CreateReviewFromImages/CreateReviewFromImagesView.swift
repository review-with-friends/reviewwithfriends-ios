...
struct CreateReviewFromImagesView: View {
    ...
    func postReview() async {
        ...
        if self.review == nil {
            ...
            let reviewResult = await belocal.createReview(token: auth.token, reviewRequest: request)
            
            switch reviewResult {
            case .success(let review):
                for selectedImage in self.selectedImages {
                    if let data = selectedImage.image.jpegData(compressionQuality: 0.9) {
                        let _ = await belocal.addReviewPic(token: auth.token, reviewId: review.id, data: data)
                    }
                }
                self.path.removeLast()
