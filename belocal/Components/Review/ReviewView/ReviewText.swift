...
struct ReviewText: View {
    ...
    func toggleFavorite() async {
        ...
        if self.isAlreadyFavorited() {
            ...
            result = await belocal.unlikeReview(token: auth.token, reviewId: self.fullReview.review.id)
        } else {
            let generator = UIImpactFeedbackGenerator(style: .light)
            generator.impactOccurred()
            
            result = await belocal.likeReview(token: auth.token, reviewId: self.fullReview.review.id)
        }
