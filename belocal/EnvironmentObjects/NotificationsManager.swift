...
class NotificationManager: ObservableObject {
    ...
    func updateDeviceToken(authToken: String, deviceToken: String) async {
        ...
        let deviceTokenUpdateResult = await belocal.setNewDeviceToken(token: authToken, deviceToken: deviceToken)
    ...
    func getNotifications(token: String) async {
        ...
        let notificationResult = await belocal.getNotifications(token: token)
        ...
        case .success(var incomingNotifications):
            ...
            let _ = await belocal.confirmNotifications(token: token)
        case .failure(_):
            return
        }
    }
