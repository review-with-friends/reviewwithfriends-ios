    ...
    func process(token: String) async -> [Review] {
        if let nextItem = self.queue.first {
            ...
            if let exclusionBoundary = nextItem.exclusionBoundary {
                result = await belocal.getReviewFromBoundaryWithExclusion(token: token, boundary: nextItem.mapBoundary, excludedBoundary: exclusionBoundary, page: nextItem.currentPage)
            } else {
                result = await belocal.getReviewFromBoundary(token: token, boundary: nextItem.mapBoundary, page: nextItem.currentPage)
            }
