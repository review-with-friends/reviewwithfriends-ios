...
struct EditReviewView: View {
    ...
    func saveReview() async {
        let result = await spotster.editReview(token: auth.token, editRequest: EditReviewRequest(review_id: self.fullReview.review.id, text: self.text, stars: self.stars))
    ...
    func addPic(data: Data) async {
        let result = await spotster.addReviewPic(token: auth.token, reviewId: self.fullReview.review.id, data: data)
    ...
    func removePic() async {
        if let pic = self.fullReview.pics.first(where: { $0.hashValue == self.selectedPhoto }) {
            let result = await spotster.removeReviewPic(token: auth.token, picId: pic.id, reviewId: self.fullReview.review.id)
            switch result {
            case .success():
                await refreshFullReview()
            case .failure(_):
